...
//
// 用户端条目顺序：仪表盘 → AI 创作 → 密钥 → 用量 → 可用渠道 → 渠道状态 → 订阅/支付 → 兑换/资料。
// 可用渠道紧挨渠道状态之上，让用户"先看自己能用什么、再看对应状态"。
function buildAiCreationNavItem(): NavItem {
  return {
    path: '/admin/ai',
    label: 'AI 创作',
    icon: SparklesIcon,
    expandOnly: true,
    children: [
      { path: '/admin/ai/chat', label: 'AI 对话', icon: ChatIcon },
      { path: '/admin/ai/image', label: 'AI 绘图', icon: SparklesIcon }
    ]
  }
}
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  if (withDashboard) {
    ...
    items.push(buildAiCreationNavItem())
  }
  items.push(
